...
document.addEventListener("DOMContentLoaded", function () {
  const scrollProgressRing = document.createElement("div");
  scrollProgressRing.classList.add("scroll-progress-ring");
  document.body.appendChild(scrollProgressRing);
  ...
  window.onscroll = function () {
    ...
    // Update progress ring border color
    const ringColor = `transparent transparent #90644e #90644e`;
    const background = `conic-gradient(${ringColor} ${
      scrolled * 3.6
    }deg, transparent ${scrolled * 3.6}deg, transparent 360deg)`;
    scrollProgressRing.style.backgroundImage = background;
  };

  scrollToTopBtn.onclick = function () {
    // Scroll to top functionality (your existing code)
